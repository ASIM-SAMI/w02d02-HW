...
let greatestSumMulti = function () {
    ...
    //     for(i=0; i < matrix[line].length; i++){
...
const check = function () {
    ...
    for (x in panagram) {
        ...
        wordArray += panagram[x];

    }
    var strToLowerCase = wordArray.toLowerCase();
    var strSpliteArray = strToLowerCase.split("");


    for (wordChar in strSpliteArray) {

        for (engChar in engAlphabet) {
            if (engAlphabet[engChar] === strSpliteArray[wordChar]) {
                        console.log(engAlphabet[engChar]);
                        checker = true;
...
const products = [
  ...
  function lessThanTen(element){
    ...
    var result = products.filter(lessThanTen).sort();
